...
class DomainHead(nn.Module):
    def __init__(self, in_features=256, hidden=128, num_domains=3):
        super().__init__()
...
class CNNLSTM_DANN(nn.Module):
    def __init__(self, num_classes, num_domains=3, grl_lambda=1.0):
        super().__init__()
...
def train_one_epoch_dann(
    ...
    for xb_s, yb_s, db_s, lb_s in source_loader:
        try:
            xb_t, yb_t, db_t, lb_t = next(target_iter)
        except StopIteration:
            target_iter = iter(target_loader)
            xb_t, yb_t, db_t, lb_t = next(target_iter)

        xb = torch.cat([xb_s, xb_t], dim=0).to(device, non_blocking=True)
        yb = torch.cat([yb_s, yb_t], dim=0).to(device, non_blocking=True)
        db = torch.cat([db_s, db_t], dim=0).to(device, non_blocking=True)
        lb = torch.cat([lb_s, lb_t], dim=0).to(device, non_blocking=True)
        ...
        # Fault loss ONLY on labeled (source) samples
        if lb.any():
...
if __name__ == "__main__":
    ...
    # Your new split: two training domains + one test domain
    Xtr, ytr, Xtr2, ytr2, Xte, yte = (
        list_of_X_y[0][0], list_of_X_y[0][1],
        list_of_X_y[1][0], list_of_X_y[1][1],
        list_of_X_y[2][0], list_of_X_y[2][1],
    )
    ...
    # Encode test labels (must be subset of training labels)
    ...
    # --- Build DANN datasets ---
    # Domain ids: 0 -> severity 0.007, 1 -> severity 0.014, 2 -> target/test domain
    ...
    ds_tgt     = DomainDataset(Xte_torch,  None,       domain_id=2, labeled=False)

    # Merge labeled source domains into one loader (simple approach)
    ds_source = torch.utils.data.ConcatDataset([ds_src_007, ds_src_014])

    # --- Optional: create a small val split from the SOURCE set only ---
    N = len(ds_source)
    idx = np.arange(N)
    np.random.seed(42)
    ...
    target_loader = DataLoader(
        ds_tgt, batch_size=64, shuffle=True, drop_last=True,
        pin_memory=True, num_workers=2, collate_fn=collate_source
    )

    # Test loader (standard: x,y)
    ...
    model = CNNLSTM_DANN(num_classes=num_classes, num_domains=3, grl_lambda=1.0).to(device)
    ...
    # --- Train ---
    epochs = 400
    domain_loss_weight = 0.8  # you can tune this

    for epoch in range(1, epochs + 1):
        loss_total, loss_f, loss_d = train_one_epoch_dann(
            model,
            source_train_loader,
            target_loader,
            optimizer,
